...
if (existsSync(envPath)) {
    ...
    process.stderr.write("⚠️ No .env file found, using environment variables if available\n");
    dotenv.config();
}
...
export function validateConfig() {
    ...
    if (notion.databaseId &&
        !/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(notion.databaseId)) {
        issues.push("NOTION_DATABASE_ID is not in valid UUID format (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)");
    }
    return {
