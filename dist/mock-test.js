...
function generateSummary(question, answer) {
    ...
    const shortQuestion = question.length > maxQuestionLength
        ? question.slice(0, maxQuestionLength) + "..."
        : question;
    const shortAnswer = answer.length > maxAnswerLength
...
function detectCodeRelated(content) {
    ...
    const lowercaseContent = content.toLowerCase();
    return codeIndicators.some((indicator) => lowercaseContent.includes(indicator));
}
function detectSolutionType(content) {
    const lowercaseContent = content.toLowerCase();
    if (lowercaseContent.includes("debug") ||
        lowercaseContent.includes("bug") ||
        lowercaseContent.includes("error") ||
        lowercaseContent.includes("fix")) {
        return "Debug";
    }
    if (lowercaseContent.includes("tool") ||
