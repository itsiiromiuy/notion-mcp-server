    ...
    try {
        ...
        if (config.notion.databaseId) {
            ...
            try {
                ...
                console.error(`✅ Successfully connected to existing database: "${database.title?.[0]?.plain_text || "Untitled"}"`);
                console.error("   No need to create a new database.");
                return;
            }
            catch (err) {
                const error = err;
                console.warn(`⚠️ Could not access configured database: ${error.message}`);
                if (error.status === 404) {
                    console.error("   The configured database was not found. Creating a new one...");
                }
                else {
        ...
        if (error.status === 404) {
            console.error("   Parent page not found. Check your NOTION_PARENT_PAGE_ID.");
        }
        else if (error.status === 403) {
            console.error("   Permission denied. Make sure your integration has write access to the parent page.");
        }
        process.exit(1);
    }
}
